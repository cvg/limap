...
// pybind11's STL casters only know std:: containers. With the BOOST hash
// backend colmap::FlatHash*/NodeHash* are boost::unordered_* instead, so the
// bindings that expose them need these. With the STD backend they are already
// std:: types and these would be redefinitions.
...
template <typename Key, typename Hash, typename Equal>
struct type_caster<colmap::NodeHashSet<Key, Hash, Equal>>
    : set_caster<colmap::NodeHashSet<Key, Hash, Equal>, Key> {};
#endif
