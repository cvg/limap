...
JLinkage::ComputeVPLabels(const std::vector<Line2d> &lines) const {
  ...
  for (int i = 0; i < n_lines; ++i) {
    if (lines[i].Length() < base_options_.min_length)
      continue;
  ...
  if (pts.size() < 2 * std::max(base_options_.min_num_supports, 10))
    return final_labels;
  ...
  int classNum = VPCluster::run(Labels, LabelCount, &pts, mModels,
                                base_options_.inlier_threshold, 2);
  ...
  for (size_t i = 0; i < LabelCount.size(); ++i) {
    ...
    if (supports.size() < base_options_.min_num_supports)
      continue;
    int num_valid_supports =
        CountValidSupports2D(supports, options_.th_perp_supports);
    if (num_valid_supports < base_options_.min_num_supports)
      continue;
    vp_ids[i] = counter++;
  }
