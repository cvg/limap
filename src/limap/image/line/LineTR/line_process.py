...
def point_on_line(line, dist_px):
    ...
    assert get_line_dist(line) >= dist_px, (
...
def line_tokenizer(
    ...
    for i, (kline, klength) in enumerate(
        zip(klines["klines"], klines["length_klines"], strict=False)
    ):
    ...
    scores = [
        s[tuple(k.t())] for s, k in zip(dense_score, pos_score, strict=False)
    ]
...
def change_cv2_T_np(klines_cv):
    klines_sp, klines_ep, length, _angle = [], [], [], []
