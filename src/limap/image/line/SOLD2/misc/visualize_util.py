...
def plot_junctions(input_image, junctions, junc_size=3, color=None):
    ...
    # A float type image ranging from 0~1
        ...
        image.dtype in [np.float32, np.float64, np.float] and image.max() <= 2.0
    ):
        image = (image * 255.0).astype(np.uint8)
    # A float type image ranging from 0.~255.
    elif (
        image.dtype in [np.float32, np.float64, np.float]
        and image.mean() > 10.0
    ):
    ...
    if junctions.shape[-1] != 2:
        if junctions.shape[0] == 2:
            ...
            raise ValueError(
                "[Error] At least one of the two dims should be 2."
            )
    ...
    junctions[junctions[:, 1] >= W, 1] = (
        W - 1
    )  # (second dim) max bounded by W-1
...
def plot_line_segments(
    ...
    for idx in range(segments.shape[0]):
        ...
        # Decide the color
        color = tuple(color) if color != "random" else tuple(np.random.rand(3))
        cv2.line(
            image,
            tuple(seg[:2]),
            tuple(seg[2:]),
    ...
    # Only plot the junctions which are part of a line segment
        ...
        for idx in range(segments.shape[0]):
            seg = np.round(segments[idx, :]).astype(
                np.int32
            )  # Already in HW format.
            cv2.circle(
                image,
                tuple(seg[:2]),
                radius=junc_size,
...
def plot_line_segments_from_segments(
    ...
    # (2) Nx2x2 format
        ...
        # Clip H dimension
        line_segments[:, :, 0] = np.clip(
            line_segments[:, :, 0], a_min=0, a_max=H - 1
        )
        line_segments[:, :, 1] = np.clip(
            line_segments[:, :, 1], a_min=0, a_max=W - 1
        )
...
def plot_keypoints(kpts, colors="lime", ps=4):
    ...
    axes = plt.gcf().axes
    for a, k, c in zip(axes, kpts, colors, strict=False):
...
def plot_lines(
    ...
    for a, l, lc, pc in zip(
        axes, lines, line_colors, point_colors, strict=False
    ):
...
def plot_color_line_matches(lines, correct_matches=None, lw=2, indices=(0, 1)):
    ...
    # Plot the lines
    for a, l in zip(axes, lines, strict=False):
        # Transform the points into the figure coordinate system
        transFigure = fig.transFigure.inverted()
...
def plot_color_lines(
    ...
    # Plot the lines
    for a, l, c in zip(axes, lines, colors, strict=False):
        # Transform the points into the figure coordinate system
        transFigure = fig.transFigure.inverted()
        endpoint0 = transFigure.transform(a.transData.transform(l[:, 0]))
        endpoint1 = transFigure.transform(a.transData.transform(l[:, 1]))
...
def plot_subsegment_matches(lines, subsegments, lw=2, indices=(0, 1)):
    ...
    # Plot the lines
    for a, l, ss in zip(axes, lines, subsegments, strict=False):
        # Transform the points into the figure coordinate system
        transFigure = fig.transFigure.inverted()
