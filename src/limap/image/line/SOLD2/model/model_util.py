...
def get_model(model_cfg=None, loss_weights=None, mode="train", printing=False):
        ...
        if loss_weights is not None:
            for param_name, param in loss_weights.items():
                if isinstance(param, nn.Parameter):
                    if printing:
                        logging.info(
                            f"\t [Debug] Adding {param_name} with value {param.item():f} to model"
                        )
    ...
    if printing:
        logging.info(
            "\tModel architecture: {}".format(model_cfg["model_architecture"])
        )
        logging.info("\tBackbone: {}".format(model_cfg["backbone"]))
        logging.info(
            "\tJunction decoder: {}".format(model_cfg["junction_decoder"])
        )
        logging.info(
            "\tHeatmap decoder: {}".format(model_cfg["heatmap_decoder"])
        )
        logging.info("\t-------------------------------------")
