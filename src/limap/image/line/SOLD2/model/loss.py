...
def get_heatmap_loss_and_weight(model_cfg, global_w_policy, device):
    ...
    if heatmap_loss_name == "cross_entropy":
        ...
        class_weight = (
            torch.tensor(np.array([1.0, heatmap_class_w]))
            .to(torch.float)
            .to(device)
        )
...
def get_descriptor_loss_and_weight(model_cfg, global_w_policy):
    ...
    if w_policy == "static":
        ...
        w_descriptor = nn.Parameter(
            torch.tensor(model_cfg["w_desc"], dtype=torch.float32),
            requires_grad=True,
        )
    else:
        raise ValueError(
    ...
    descriptor_loss_name = model_cfg.get(
...
def space_to_depth(input_tensor, grid_size):
    ...
    # (N, C, H//bs, bs, W//bs, bs)
    x = input_tensor.view(
        N, C, H // grid_size, grid_size, W // grid_size, grid_size
    )
    # (N, bs, bs, C, H//bs, W//bs)
...
def junction_detection_loss(
    ...
    valid_mask = (
        torch.ones(junction_map.shape) if valid_mask is None else valid_mask
    )
    ...
    if keep_border:
        valid_mask = (
            torch.sum(
                valid_mask.to(torch.bool).to(torch.int), dim=1, keepdim=True
            )
            > 0
        )
    else:
    ...
    loss_final = loss_ / torch.sum(
        torch.squeeze(valid_mask.to(torch.float), dim=1)
    )
...
class TotalLoss(nn.Module):
    ...
    def forward(
        ...
        junc_loss = self.loss_funcs["junc_loss"](
            junc_pred, junc_target, valid_mask
        )
    ...
    def forward_descriptors(
        ...
        outputs = {
            ...
            "heatmap_loss": heatmap_loss,
            "w_junc": (
                w_junc.item() if isinstance(w_junc, nn.Parameter) else w_junc
            ),
            "w_heatmap": (
                w_heatmap.item()
