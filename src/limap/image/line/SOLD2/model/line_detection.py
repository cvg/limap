...
class LineSegmentDetectionModule:
    ...
    def detect(self, junctions, heatmap, device=None):
        ...
        detection_results = (
            torch.mean(sampled_feat, dim=-1) > self.detect_thresh
        )

        # [Inlier threshold detection]
        if self.inlier_thresh > 0.0:
            inlier_ratio = (
                torch.sum(sampled_feat > self.detect_thresh, dim=-1).to(
                    torch.float32
                )
                / self.num_samples
            )
        ...
        line_map_pred[
            detected_junc_indexes[:, 0], detected_junc_indexes[:, 1]
        ] = 1
        line_map_pred[
            detected_junc_indexes[:, 1], detected_junc_indexes[:, 0]
        ] = 1
    ...
    def refine_heatmap_local(
        ...
        count_map = torch.zeros(
            heatmap.shape, dtype=torch.int, device=heatmap.device
        )
        ...
        heatmap_output = torch.clamp(
            heatmap_output / count_map, max=1.0, min=0.0
        )
    ...
    def candidate_suppression(self, junctions, candidate_map):
        ...
        line_dist_map = (
            torch.sum(
                (torch.unsqueeze(junctions, dim=1) - junctions[None, ...]) ** 2,
                dim=-1,
            )
            ** 0.5
        )
        ...
        junc_counts -= junc_mask[..., 0][
            torch.arange(0, num_segs), end_point_idxs
        ].to(torch.int)

        # Get the invalid candidate mask
        final_mask = junc_counts > 0
        candidate_map[
            start_point_idxs[final_mask], end_point_idxs[final_mask]
        ] = 0

        return candidate_map

    def refine_junction_perturb(
        self, junctions, line_map_pred, heatmap, H, W, device
    ):
        ...
        if num_segments == 0:
            refined_segments = line_segments
        else:
        ...
        line_map_new = self.segments_to_line_map(
            junctions_new, refined_segments
        )
    ...
    def segments_to_line_map(self, junctions, segments):
        ...
        for idx in range(segments.shape[0]):
            ...
            idx_junction1 = torch.where(
                (junctions == junction1).sum(axis=1) == 2
            )[0]
            idx_junction2 = torch.where(
                (junctions == junction2).sum(axis=1) == 2
            )[0]
    ...
    def detect_local_max(
        ...
        patch_points_shifted = (
            torch.unsqueeze(cand_points_round, dim=2)
            + patch_points[None, None, ...]
        )
        patch_dist = torch.sqrt(
            torch.sum(
                (torch.unsqueeze(cand_points, dim=2) - patch_points_shifted)
                ** 2,
                dim=-1,
            )
        )
        ...
        points_H = torch.clamp(
            patch_points_shifted[:, :, :, 0], min=0, max=H - 1
        ).to(torch.long)
        points_W = torch.clamp(
            patch_points_shifted[:, :, :, 1], min=0, max=W - 1
        ).to(torch.long)
