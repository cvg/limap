...
class SOLD2Detector(BaseDetector):
    ...
    def extract_heatmap(self, image_path: Path):
        img = cv2.imread(image_path)
        if self.set_gray:
            img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        segs, descriptor, heatmap, descinfo = self.detector.detect(img)
    ...
    def extract_heatmaps_all_images(
        self, folder, image_paths, skip_exists=False
    ):
        ...
        if not (
            skip_exists
            and os.path.exists(os.path.join(heatmap_folder, "image_paths.npy"))
        ):
            limapio.save_npy(
                os.path.join(heatmap_folder, "image_paths.npy"),
                image_paths,
            )
        for img_id, image_path in tqdm(image_paths.items()):
            ...
            if self.set_gray:
                img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            heatmap = self.extract_heatmap(img)
