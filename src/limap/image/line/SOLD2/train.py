...
def train_single_epoch(
    ...
    for idx, data in enumerate(train_loader):
        ...
        if (idx % model_cfg["disp_freq"]) == 0:
            ...
            logging.info(
                "\t Junction     precision={:.4f} ({:.4f}) / recall={:.4f} ({:.4f})".format(
            ...
            logging.info(
                "\t Junction nms precision={:.4f} ({:.4f}) / recall={:.4f} ({:.4f})".format(
            ...
            logging.info(
                "\t Heatmap      precision={:.4f} ({:.4f}) / recall={:.4f} ({:.4f})".format(
            ...
            if compute_descriptors:
                logging.info(
                    "\t Descriptors  matching score={:.4f} ({:.4f})".format(
...
def record_train_summaries(writer, global_step, scalars, images):
    ...
    if "descriptor_loss" in scalars:
        key = "descriptor_loss"
        writer.add_scalar(f"Train_loss/{key}", scalars[key], global_step)
        writer.add_scalar(
            f"Train_loss_average/{key}", average[key], global_step
        )

    # Record weighting
    for key in scalars:
        if "w_" in key:
            writer.add_scalar(f"Train_weight/{key}", scalars[key], global_step)
...
def record_test_summaries(writer, epoch, scalars):
    ...
    if "descriptor_loss" in average:
        key = "descriptor_loss"
        writer.add_scalar(f"Val_loss/{key}", average[key], epoch)
