...
def run_incremental_line_triangulation(
    ...
    This simulates incremental SfM by:
    1. Loading the structure database (with detections and matches)
    2. Processing images one at a time in order
    3. Triangulating lines incrementally as each image is "registered"
    ...
    # Configure incremental triangulator options
    options = limap.sfm.IncrementalLineTriangulatorOptions()
    for key, value in cfg.get("incremental_line_triangulation", {}).items():
    ...
    logging.info(
        f"Running incremental line triangulation on "
        f"{sum(1 for img in point_recon.images.values() if img.has_pose)} "
        f"images..."
    )

    _sfm.incremental_line_triangulation_pipeline(
        structure_db_cache,
        holistic_recon,
        options,
    ...
    logging.info("Incremental line triangulation complete.")
...
def parse_args():
    arg_parser = argparse.ArgumentParser(
        description="Run incremental line triangulation"
    )
...
def main():
    args, cfg = parse_args()
    run_incremental_line_triangulation(
