...
def run_point_line_triangulation_from_colmap(cfg, model_path, image_path):
    output_dir = Path(cfg["output_dir"])

    # Disable groups - only triangulate points and lines
...
def parse_config():
    ...
    arg_parser = argparse.ArgumentParser(
        description="triangulate 3D points and lines from COLMAP (no groups)"
    )
...
def main():
    cfg = parse_config()
    run_point_line_triangulation_from_colmap(
