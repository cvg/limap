"""Base reader classes for depth maps and point clouds."""
...
class BasePointCloudReader:
    """
    Base class for point cloud readers storing the filename and
    potentially other information.

    The point cloud should be in world coordinates and organized as an
    image-like array (H, W, 3) for pixel-based lookup during line fitting.
    """
    ...
    def read(self, filename):
        ...
        Returns:
            point cloud (:class:`np.array` of shape (H, W, 3)):
                The array for the 3D points in world coordinates,
                organized as an image for pixel-based lookup
        """
        raise NotImplementedError
