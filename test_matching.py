...
detector = limap.line2d.get_detector({"method": "lsd", "skip_exists": False}) # get a line detector
extractor = limap.line2d.get_extractor({"method": "dense_naive", "skip_exists": False}) # get a line extractor
matcher = limap.line2d.get_matcher({"method": "dense_roma", "skip_exists": False, "n_jobs": 1, "topk": 0}, extractor) # initiate a line matcher

view1 = limap.base.CameraView("/home/shaoliu/workspace/GlueStick/resources/img1.jpg") # initiate an limap.base.CameraView instance for detection.
view2 = limap.base.CameraView("/home/shaoliu/workspace/GlueStick/resources/img2.jpg") # initiate an limap.base.CameraView instance for detection.
...
def vis_matches(img1, img2, segs1, segs2, matches):
    ...
    for idx in range(n_lines):
        color = np.array(colors[idx]) * 255.
        color = color.astype(int).tolist()
        cv2.line(img1_draw, (int(matched_seg1[idx, 0]), int(matched_seg1[idx, 1])), (int(matched_seg1[idx, 2]), int(matched_seg1[idx, 3])), color, 4)
        cv2.line(img2_draw, (int(matched_seg2[idx, 0]), int(matched_seg2[idx, 1])), (int(matched_seg2[idx, 2]), int(matched_seg2[idx, 3])), color, 4)
