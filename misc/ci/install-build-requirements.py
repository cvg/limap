...
cibuildwheel is run with `--no-isolation` so that the compiler include paths
stay stable across wheels; a fresh isolation directory changes them on every
build and defeats ccache. Adapted from colmap/colmap (BSD-3-Clause).
"""
