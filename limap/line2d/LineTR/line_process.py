...
def filter_by_length(lines, min_length, max_sublines):
    ...
    return {'lines': klines, 'length_klines': length, 'angles': angles}
...
def line_tokenizer(klines, token_distance, max_tokens, pred_superpoint, image_shape):
    ...
    for i, (kline, klength) in enumerate(zip(klines['lines'], klines['length_klines'])):
    ...
    device = pred_superpoint['all_descriptors'].device
    # device = 'cpu'
    ...
    klines2slines = torch.zeros((len(klines['lines']), len(slines))).to(device)
    st_idx = 0
    ...
    dense_descriptor = pred_superpoint['all_descriptors']
    descriptors = sample_descriptors(tokens[None], dense_descriptor, 8)[0].reshape(256, tokens.shape[0],
                                                                                   tokens.shape[1])
    ...
    # numpy2tensor
    klines['lines'] = torch.from_numpy(klines['lines']).float().to(device)[None]
