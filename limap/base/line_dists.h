#ifndef LIMAP_BASE_LINE_DISTS_H
#define LIMAP_BASE_LINE_DISTS_H
...
double compute_distance(const LineType& l1, const LineType& l2, const LineDistType& type);

template <typename LineType>
Eigen::MatrixXd compute_pairwise_distance(const std::vector<LineType> lines, const LineDistType& type) {
    size_t n = lines.size();
...
double dist_midpoint_perpendicular(const LineType& l1, const LineType& l2) {
    ...
    double d12squared = (mid1 - l2.start).squaredNorm() - pow((mid1 - l2.start).dot(v2), 2);
    double d12 = sqrt(std::max(d12squared, 0.0));
    double d21squared = (mid2 - l1.start).squaredNorm() - pow((mid2 - l1.start).dot(v1), 2);
...
template <typename LineType>
double dist_endpoints_perpendicular_oneway(const LineType& l1, const LineType& l2) {
    std::pair<double, double> dists = dists_endpoints_perpendicular_oneway<LineType>(l1, l2);
    return std::max(dists.first, dists.second);
}

template <typename LineType>
std::vector<double> dists_endpoints_perpendicular(const LineType& l1, const LineType& l2) {
