...
double dist_endpoints_perpendicular_scaleinv_line3dpp_oneway(const Line3d& l1, const Line3d& l2) {
    assert(l1.depths[0] != -1 && l1.depths[1] != -1);
    assert(l2.depths[0] != -1 && l2.depths[1] != -1);
    std::pair<double, double> dists = dists_endpoints_perpendicular_oneway<Line3d>(l1, l2);
    return std::max(dists.first / (l1.depths[0] + EPS), dists.second / (l1.depths[1] + EPS));
}

double dist_endpoints_perpendicular_scaleinv_line3dpp(const Line3d& l1, const Line3d& l2) {
...
double dist_endpoints_perpendicular_scaleinv_oneway(const Line3d& l1, const Line3d& l2) {
    assert(l1.depths[0] != -1 && l1.depths[1] != -1);
    assert(l2.depths[0] != -1 && l2.depths[1] != -1);
    std::pair<double, double> dists = dists_endpoints_perpendicular_oneway<Line3d>(l1, l2);
    double dist_start = dists.first; double dist_end = dists.second;

    V3D dir_l2 = l2.direction();
    double length_l2 = l2.length();
    double alpha_start = (l1.start - l2.start).dot(dir_l2) / length_l2;
    double depth_start = l2.depths[0] + alpha_start * (l2.depths[1] - l2.depths[0]);
...
double dist_endpoints_perpendicular_scaleinv(const Line3d& l1, const Line3d& l2) {
...
double dist_endpoints_scaleinv_oneway(const Line3d& l1, const Line3d& l2) {
    ...
    return std::max(dist_start / (l1.depths[0] + EPS), dist_end / (l1.depths[1] + EPS));
}
...
double infinite_dist_perpendicular(const Line3d& l1, const Line3d& l2) {
    ...
    A11 = Cp.dot(Cp); A22 = Cq.dot(Cq);
    A12 = A21 = Cp.dot(Cq);
    B1 = -C0.dot(Cp); B2 = -C0.dot(Cq);
...
double infinite_perpendicular_scaleinv_line3dpp(const Line3d& l1, const Line3d& l2) {
    ...
    assert(l1.depths[0] != -1 && l1.depths[1] != -1);
    double z1, z2; z1 = l1.depths[0]; z2 = l1.depths[1];
...
template<>
double compute_distance<Line2d>(const Line2d& l1, const Line2d& l2, const LineDistType& type) {
    switch (type) {
        ...
        case LineDistType::PERPENDICULAR_SCALEINV_ONEWAY:
            throw std::runtime_error("Type error. Scale invariance perpendicular distance is not supported for Line2d.");
...
template<>
double compute_distance<Line3d>(const Line3d& l1, const Line3d& l2, const LineDistType& type) {
    switch (type) {
        case LineDistType::ANGULAR:
