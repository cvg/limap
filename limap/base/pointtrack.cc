...
PointTrack::PointTrack(const PointTrack& track) {
    ...
    std::copy(track.p2d_id_list.begin(), track.p2d_id_list.end(), std::back_inserter(p2d_id_list));
    std::copy(track.p2d_list.begin(), track.p2d_list.end(), std::back_inserter(p2d_list));
}

py::dict PointTrack::as_dict() const {
    py::dict output;
    output["p"] = p;
    output["point_id"] = point_id;
...
PointTrack::PointTrack(py::dict dict) {
    ...
    ASSIGN_PYDICT_ITEM(dict, point_id, int)
