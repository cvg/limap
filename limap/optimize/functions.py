...
def visualize_heatmap_intersections(
    prefix,
    imname_list,
    image_ids,
    p_heatmaps,
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    for img_id, heatmap, intersections in zip(
        image_ids, p_heatmaps, ht_intersections
    ):
        ...
        # visualize heatmap
        heatmap_img = (scalarMap.to_rgba(heatmap)[:, :, :3] * 255).astype(
            np.uint8
        )
...
def visualize_fconsis_intersections(
    prefix,
    imname_list,
    image_ids,
    fc_intersections,
    ...
    for sample_id, intersections in enumerate(tqdm(fc_intersections)):
        ...
        for data in intersections:
            ...
            img = utils.read_image(
                imname_list[img_id], max_image_dim=max_image_dim, set_gray=False
            )
