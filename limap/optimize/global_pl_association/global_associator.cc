...
void GlobalAssociator::InitVPTracks(const std::map<int, vplib::VPTrack>& vptracks) {
    vps_.clear();
    vp_tracks_.clear();
...
void GlobalAssociator::InitVPTracks(const std::vector<vplib::VPTrack>& vptracks) {
    vps_.clear();
    vp_tracks_.clear();
...
void GlobalAssociator::ReassociateJunctions() {
    ...
    for (const auto& kv: line_tracks_) {
        ...
        for (size_t support_id = 0; support_id < n_supports; ++support_id) {
            ...
            if (m_lineid_2dto3d.at(img_id).find(line2d_id) == m_lineid_2dto3d.at(img_id).end())
    ...
    std::map<std::pair<int, int>, std::vector<std::pair<int, int>>> track_edges_counter; // ((linetrack_id1, linetrack_id2), vector<img_id, point2d_id>)
    for (auto it_bpt2d = all_bpt2ds_.begin(); it_bpt2d != all_bpt2ds_.end(); ++it_bpt2d) {
        ...
        for (const int& point2d_id: bpt2d.get_point_ids()) {
            ...
            for (size_t i = 0; i < n_lines - 1; ++i) {
                ...
                if (m_lineid_2dto3d.at(img_id).find(line2d_id1) == m_lineid_2dto3d.at(img_id).end())
                    continue;
                ...
                for (size_t j = i + 1; j < n_lines; ++j) {
                    ...
                    if (m_lineid_2dto3d.at(img_id).find(line2d_id2) == m_lineid_2dto3d.at(img_id).end())
                        continue;
                    ...
                    if (track_edges_counter.find(edge) == track_edges_counter.end()) {
                        track_edges_counter.insert(std::make_pair(edge, std::vector<std::pair<int, int>>()));
                    }
                    track_edges_counter.at(edge).push_back(std::make_pair(img_id, point2d_id));
                }
            }
        }
    }
    ...
    // build junctions
    for (auto it = track_edges_counter.begin(); it != track_edges_counter.end(); ++it) {
        ...
        if (angle < config_.th_angle_lineline)
            continue;
        // compute midpoint
        M2D A; A << 1.0, -inner_prod, -inner_prod, 1.0;
        V2D b;
        b(0) = direc1.dot(line2.start - line1.start);
        b(1) = direc2.dot(line1.start - line2.start);
        V2D res = A.inverse() * b;
        V3D point = 0.5 * (line1.start + res[0] * direc1 + line2.start + res[1] * direc2);
...
void GlobalAssociator::SetUp() {
    ...
    if (enable_pointline_ && (!config_.constant_point || !config_.constant_line)) {
...
void GlobalAssociator::AddPointLineAssociationResiduals(const std::map<std::pair<int, int>, double>& weights) {
    if (config_.lw_pointline_association <= 0)
        return;
    ceres::LossFunction* loss_function = config_.point_line_association_3d_loss_function.get();
    for (auto it = weights.begin(); it != weights.end(); ++it) {
        int point3d_id = it->first.first;
        int line3d_id = it->first.second;
        double weight = it->second;

        ceres::CostFunction* cost_function = PointLineAssociation3dFunctor::Create();
        ceres::LossFunction* scaled_loss_function = new ceres::ScaledLoss(loss_function, weight * config_.lw_pointline_association, ceres::DO_NOT_TAKE_OWNERSHIP);
        ceres::ResidualBlockId block_id = problem_->AddResidualBlock(cost_function, scaled_loss_function, 
                                                                     points_.at(point3d_id).data(), lines_.at(line3d_id).uvec.data(), lines_.at(line3d_id).wvec.data());
    }
}

void GlobalAssociator::AddVPLineAssociationResiduals(const std::map<std::pair<int, int>, int>& weights) {
    if (config_.lw_vpline_association <= 0)
        return;
    ceres::LossFunction* loss_function = config_.vp_line_association_3d_loss_function.get();
    for (auto it = weights.begin(); it != weights.end(); ++it) {
        ...
        ceres::LossFunction* scaled_loss_function = new ceres::ScaledLoss(loss_function, double(weight) * 1e2 * config_.lw_vpline_association, ceres::DO_NOT_TAKE_OWNERSHIP);
        ceres::ResidualBlockId block_id = problem_->AddResidualBlock(cost_function, scaled_loss_function, 
                                                                     vps_.at(vp3d_id).data(), lines_.at(line3d_id).uvec.data(), lines_.at(line3d_id).wvec.data());
    }
} 
...
void GlobalAssociator::AddVPOrthogonalityResiduals() {
    if (config_.lw_vp_orthogonality <= 0)
        return;
    ceres::LossFunction* loss_function = config_.vp_orthogonality_loss_function.get();
    auto pairs = construct_pairs_vp_orthogonality(vps_, config_.th_angle_orthogonality);
    for (auto it = pairs.begin(); it != pairs.end(); ++it) {
        int id1 = it->first;
        int id2 = it->second;

        ceres::CostFunction* cost_function = VPOrthogonalityFunctor::Create();
        ceres::LossFunction* scaled_loss_function = new ceres::ScaledLoss(loss_function, 1e2 * config_.lw_vp_orthogonality, ceres::DO_NOT_TAKE_OWNERSHIP);
        ceres::ResidualBlockId block_id = problem_->AddResidualBlock(cost_function, scaled_loss_function, 
...
void GlobalAssociator::AddVPCollinearityResiduals() {
    if (config_.lw_vp_collinearity <= 0)
        return;
    ceres::LossFunction* loss_function = config_.vp_collinearity_loss_function.get();
    auto pairs = construct_pairs_vp_collinearity(vps_, config_.th_angle_collinearity);
    for (auto it = pairs.begin(); it != pairs.end(); ++it) {
        int id1 = it->first;
        int id2 = it->second;

        ceres::CostFunction* cost_function = VPCollinearityFunctor::Create();
        ceres::LossFunction* scaled_loss_function = new ceres::ScaledLoss(loss_function, 1e2 * config_.lw_vp_collinearity, ceres::DO_NOT_TAKE_OWNERSHIP);
        ceres::ResidualBlockId block_id = problem_->AddResidualBlock(cost_function, scaled_loss_function, 
                                                                     vps_.at(id1).data(), vps_.at(id2).data());
    }
}
...
double GlobalAssociator::compute_overall_weight(const std::vector<std::pair<int, double>>& dists) const {
...
std::map<std::pair<int, int>, double> GlobalAssociator::construct_weights_pointline(const double th_weight) const 
{
    std::map<std::pair<int, int>, std::vector<std::pair<int, double>>> dists_collection; // (point3d_id, line3d_id), (img_id, dist)
    for (const auto& kv: line_tracks_) {
        int line3d_id = kv.first;
        const LineTrack& linetrack = line_tracks_.at(line3d_id);
        size_t n_supports = linetrack.count_lines();
        for (size_t support_id = 0; support_id < n_supports; ++support_id) {
            int img_id = linetrack.image_id_list[support_id];
            int line2d_id = linetrack.line_id_list[support_id];
            std::vector<int> point2d_ids = all_bpt2ds_.at(img_id).neighbor_points(line2d_id);
            for (const int& point2d_id: point2d_ids) {
                ...
                if (dists_collection.find(edge_index) == dists_collection.end())
                    dists_collection.insert(std::make_pair(edge_index, std::vector<std::pair<int, double>>()));
...
std::map<std::pair<int, int>, int> GlobalAssociator::construct_weights_vpline(const int th_count) const {
    std::map<int, std::map<int, int>> edge_counter; // (line3d_id, (vp3d_id, count))
    for (const auto& kv: line_tracks_) {
        int line3d_id = kv.first;
        const LineTrack& linetrack = line_tracks_.at(line3d_id);
        size_t n_supports = linetrack.count_lines();
        for (size_t support_id = 0; support_id < n_supports; ++support_id) {
            int img_id = linetrack.image_id_list[support_id];
            int line2d_id = linetrack.line_id_list[support_id];
            std::vector<int> vp2d_ids = all_bpt2ds_vp_.at(img_id).neighbor_points(line2d_id);
            if (vp2d_ids.empty())
                continue;
            ...
            if (edge_counter.at(line3d_id).find(vp3d_id) == edge_counter.at(line3d_id).end())
    ...
    std::map<int, std::pair<int, int>> best_assignments; // (line3d_id, (best_vp3d_id, max_count))
    for (auto it = edge_counter.begin(); it != edge_counter.end(); ++it) {
        int line3d_id = it->first;
        int best_vp3d_id = -1;
        int best_count = 0;
        for (auto it2 = edge_counter.at(line3d_id).begin(); it2 != edge_counter.at(line3d_id).end(); ++it2) {
        ...
        best_assignments.insert(std::make_pair(line3d_id, std::make_pair(best_vp3d_id, best_count)));
    }
...
std::vector<std::pair<int, int>> GlobalAssociator::construct_pairs_vp_orthogonality(const std::map<int, V3D>& vps, const double th_angle_orthogonality) const {
...
std::vector<std::pair<int, int>> GlobalAssociator::construct_pairs_vp_collinearity(const std::map<int, V3D>& vps, const double th_angle_collinearity) const {
    std::vector<std::pair<int, int>> pairs;
    std::vector<int> vp3d_ids;
    for (auto it = vps.begin(); it != vps.end(); ++it) {
        vp3d_ids.push_back(it->first);
...
structures::PL_Bipartite3d GlobalAssociator::GetBipartite3d_PointLine_Constraints() const {
    // init bipartite
    ...
    for (auto it = lines_.begin(); it != lines_.end(); ++it) {
        ...
        Line3d line = GetLineSegmentFromInfiniteLine3d(inf_line, views, ltrack.line2d_list);
        ltrack.line = line;
...
structures::PL_Bipartite3d GlobalAssociator::GetBipartite3d_PointLine() const {
    ...
    for (auto it = weights.begin(); it != weights.end(); ++it) {
        ...
        if (!bpt.exist_point(point3d_id) || !bpt.exist_line(line3d_id))
            continue;
        double dist = bpt.line(line3d_id).line.point_distance(bpt.point(point3d_id).p);
        // test on 3d
...
structures::VPLine_Bipartite3d GlobalAssociator::GetBipartite3d_VPLine() const {
    ...
    for (auto it = weights.begin(); it != weights.end(); ++it) {
        ...
        if (!bpt.exist_point(vp3d_id) || !bpt.exist_line(line3d_id))
            continue;
        // test on 3d
        double cosine = std::abs(bpt.point(vp3d_id).direction.dot(bpt.line(line3d_id).line.direction()));
        if (cosine > 1.0)
            cosine = 1.0;
        double angle = acos(cosine) * 180.0 / M_PI;
