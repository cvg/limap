...
// Idea: Simple logger that can:
//  -> write to cout (level0), cerr(level1), redirect each of these streams to a
//  file
//  -> has two special levels (level2 = cout, level3=cerr), that can even write
//  to these files if parallel
//  -> Interfaces nicely with pybind11
...
public:
  ...
  template <class T> STDLOG &operator<<(const T &msg) {
...
public:
  ...
  template <class T> SYNC_LOG &operator<<(const T &msg) {
