...
// Copied from the pixel-perfect-sfm project
...
void __ThrowCheckOpImpl(const char *file, const int line, const bool result,
  ...
  if (!result) {
    ...
    throw TemplateException<py::value_error>(
        file, line, __GetCheckString(msg.c_str()).c_str());
  }
}
