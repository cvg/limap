...
std::vector<int> ComputeLineTrackLabelsGreedy(const Graph& graph, const std::vector<Line3d>& line3d_list_nodes, LineLinker3d linker3d)
{
    ...
    for (Edge* edge : graph.undirected_edges) {
        edges.push_back(std::make_tuple(edge->sim, edge->node_idx1, edge->node_idx2));
    }
    STDLOG(INFO) << "# graph nodes:" << " " << n_nodes << std::endl;
    STDLOG(INFO) << "# graph edges:" << " " << edges.size() * 2 << std::endl;
    ...
    STDLOG(INFO) << "# tracks:" << " " << n_tracks << std::endl;
...
std::vector<int> ComputeLineTrackLabelsExhaustive(const Graph& graph, const std::vector<Line3d>& line3d_list_nodes, LineLinker3d linker3d)
{
    // set the mode to avgtest merging
    linker3d.config.set_to_avgtest_merging();
    const size_t n_nodes = graph.nodes.size();
    ...
    for (size_t edge_id = 0; edge_id < n_edges; ++edge_id) {
        ...
        if (root1 != root2) {
            ...
            bool flag = true;
            for (auto it1 = lines_in_track[root1].begin(); it1 != lines_in_track[root1].end(); ++it1) {
                for (auto it2 = lines_in_track[root2].begin(); it2 != lines_in_track[root2].end(); ++it2) {
            ...
            if (images_in_track[root1].size() < images_in_track[root2].size()) {
                ...
                lines_in_track[root2].insert(lines_in_track[root2].end(), lines_in_track[root1].begin(), lines_in_track[root1].end());
                ...
                lines_in_track[root1].insert(lines_in_track[root1].end(), lines_in_track[root2].begin(), lines_in_track[root2].end());
...
std::vector<int> ComputeLineTrackLabelsAvg(const Graph& graph, const std::vector<Line3d>& line3d_list_nodes, LineLinker3d linker3d)
{
    // set the mode to avgtest merging
    linker3d.config.set_to_avgtest_merging();
    const size_t n_nodes = graph.nodes.size();
    ...
    for (size_t edge_id = 0; edge_id < num_edges; ++edge_id) {
        ...
        if (root1 != root2) {
            // test the similarity between the two unions
            if (!linker3d.check_connection(avgline_in_track[root1].first, avgline_in_track[root2].first))
                continue;
            ...
            if (images_in_track[root1].size() < images_in_track[root2].size()) {
                ...
                Line3d newline;
                newline.start = (d1.first.start * d1.second + d2.first.start * d2.second) / (d1.second + d2.second);
                newline.end = (d1.first.end * d1.second + d2.first.end * d2.second) / (d1.second + d2.second);
                avgline_in_track[root2] = std::make_pair(newline, d1.second + d2.second);
            } 
            else {
                parent_nodes[root2] = root1;
                // update images_in_track for root1
                ...
                newline.start = (d1.first.start * d1.second + d2.first.start * d2.second) / (d1.second + d2.second);
                newline.end = (d1.first.end * d1.second + d2.first.end * d2.second) / (d1.second + d2.second);
                avgline_in_track[root1] = std::make_pair(newline, d1.second + d2.second);
            }
        }
    }
...
#pragma omp parallel for
            ...
            for (size_t ng_id = 0; ng_id < n_neighbors; ++ng_id) {
                ...
                for (size_t ng_line_id = 0; ng_line_id < ng_n_lines; ++ng_line_id) {
                    ...
                    if (!linker.check_connection_3d(l1, l2))
                        continue;
                    // check 2d
                    if (!linker.check_connection_2d(l1.projection(cameras[ng_image_id]), all_lines_2d[ng_image_id][ng_line_id]))
                        continue;
                    if (!linker.check_connection_2d(l2.projection(cameras[image_id]), all_lines_2d[image_id][line_id]))
                        continue;
    ...
    for (auto it = linetracks.begin(); it != linetracks.end(); ++it) {
        it->line = Aggregator::aggregate_line3d_list(it->line3d_list, it->score_list, 0);
    }
}
...
#pragma omp parallel for
    ...
    for (auto it = edges.begin(); it != edges.end(); ++it) {
        ...
        if (root1 != root2) {
            ...
            if (tracks_in_group[root1].size() < tracks_in_group[root2].size()) {
                ...
                tracks_in_group[root2].insert(
                    tracks_in_group[root1].begin(), tracks_in_group[root1].end());
                tracks_in_group[root1].clear();
            } 
            else {
                parent_tracks[root2] = root1;
                // update tracks_in_group for root1
                tracks_in_group[root1].insert(
                  tracks_in_group[root2].begin(), tracks_in_group[root2].end());
    ...
    for (size_t track_idx = 0; track_idx < n_tracks; ++track_idx) {
        ...
        group_labels[track_idx] = group_labels[union_find_get_root(track_idx, parent_tracks)];
    }
    ...
    for (size_t track_id = 0; track_id < n_tracks; ++track_id) {
        ...
        new_linetracks[group_id].node_id_list.insert(new_linetracks[group_id].node_id_list.end(), track.node_id_list.begin(), track.node_id_list.end());
        new_linetracks[group_id].image_id_list.insert(new_linetracks[group_id].image_id_list.end(), track.image_id_list.begin(), track.image_id_list.end());
        new_linetracks[group_id].line_id_list.insert(new_linetracks[group_id].line_id_list.end(), track.line_id_list.begin(), track.line_id_list.end());
        new_linetracks[group_id].line2d_list.insert(new_linetracks[group_id].line2d_list.end(), track.line2d_list.begin(), track.line2d_list.end());
        new_linetracks[group_id].line3d_list.insert(new_linetracks[group_id].line3d_list.end(), track.line3d_list.begin(), track.line3d_list.end());
        new_linetracks[group_id].score_list.insert(new_linetracks[group_id].score_list.end(), track.score_list.begin(), track.score_list.end());
    }
